...
function updateVisibleOrderCount() {
    ...
    let countDisplay = document.querySelector('.filteredCount');
    if (!countDisplay) {
        countDisplay = document.createElement('div');
        countDisplay.className = 'filteredCount';
        
        // Find a better place to insert it - perhaps near the duration filter
        const filterContainer = document.querySelector('.oneStatFilter') || 
                              document.querySelector('.durationFilter').parentNode;
        if (filterContainer) {
            filterContainer.appendChild(countDisplay);
        } else {
            console.error('Could not find container for filtered count');
            return;
        }
    }
