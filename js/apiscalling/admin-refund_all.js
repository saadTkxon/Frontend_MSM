async function fetchRefundData() {
    try {
        ...
        if (data.status && data.refunds && data.refunds.length > 0) {
            console.log('Displaying refund data...'); // Debug log
            displayRefundData(data.refunds);
        } else {
...
function displayRefundData(refunds) {
    ...
    refunds.forEach(refund => {
        ...
        let statusText = refund.status;
        let statusClass = refund.status;
...
// Update the close function to match
...
//     }
//     // Inside displayOrderDetails function, add this:
// if (orderData.refund) {
...
//     // Set voucher information (if available in API)
//     // updateElementText('.orderBillDets .oneDet:nth-child(3) h2 span', 'VN256');
//     // updateElementText('.orderBillDets .oneDet:nth-child(3) h1 span', '280');
...
function displayOrderDetails(orderData) {
    ...
    if (orderData.refund) {
        ...
        // Update cancelled by information - THIS IS THE NEW PART YOU WANTED
       // Update cancelled by information - THIS IS THE NEW PART YOU WANTED
const cancelledBy = orderData.order.cancelled_by === 'admin' ? 'admin' : 'user';
updateElementHTML('.cancelOrderReason h2 small:nth-child(1)', 
    `Cancelled by <span>${cancelledBy}</span>`);
            ...
            <td>
                <div class="orderActionArea" data-refund-id="${refund.refund_id}">
                    <select class="statusSelect">
                        ...
                        <option value="paid" ${refund.status === 'paid' ? 'selected' : ''}>Set as paid</option>
                        <option value="rejected" ${refund.status === 'rejected' ? 'selected' : ''}>Set as rejected</option>
                    </select>
...
// At the bottom of your file, with the other window declarations
