...
function renderOrders(orders) {
    ...
    sortedOrders.forEach((order, index) => {
        ...
        // Check if order status is refund_apply
        const isRefundApply = status === 'refund_apply';
...
async function openOrderDetails(orderId, status) {
    ...
    switch(status.toLowerCase()) {
        ...
        case 'failed delivery':
            ...
            break;
        case 'cancelled':
            case 'refund_apply': // Use cancelled panel for refund_apply as it's similar
            panelSelector = '.cancelledOrderDets';
            break;
        case 'returned':
            panelSelector = '.returnedOrderDets';
...
// // Payment method
// let paymentMethod;
// if (order.status === 'online_payment') {
...
// Update the display with both payment method and reference (if available)
panel.querySelector('.orderBillDets .oneDet:nth-child(6) h1').textContent = 
    `${paymentMethod.toLowerCase()}${paymentReference}`;
...
// Hide cancellation section for refund_apply status
if (status.toLowerCase() === 'refund_apply') {
    const cancelOrderSection = panel.querySelector('.cancelOrder');
    ...
    const cancelOrderSection = panel.querySelector('.cancelOrder');
    if (cancelOrderSection) {
        cancelOrderSection.style.display = 'block'; // Or whatever default display it should have
    }
}
...
// In your populateOrderDetails function, replace the status check with:
if (status.toLowerCase() === 'in-process' || status.toLowerCase() === 'online_payment' ||  status.toLowerCase() === 'placed') {
 ...
 if (statusesWithTracking.includes(status.toLowerCase())) {
  ...
  // Check if status is refund_apply
  const isRefundApply = status.toLowerCase() === 'refund_apply';
    
  // Hide status change controls for refund_apply
  const statusActionArea = panel.querySelector('.orderActionArea');
  if (statusActionArea && isRefundApply) {
      statusActionArea.style.display = 'none';
  }

  // Hide tracking form for refund_apply
  const trackingForm = panel.querySelector('.addTracking');
  if (trackingForm && isRefundApply) {
      trackingForm.style.display = 'none';
  }

  // For refund_apply orders, just show tracking info if it exists
  if (isRefundApply) {
      const trackingInfo = await fetchTrackingDetails(order.order_id);
      const trackingContainer = panel.querySelector('.orderTracking');
